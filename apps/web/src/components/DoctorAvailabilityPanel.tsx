...
export function DoctorAvailabilityPanel() {
  ...
  const handleSaveRules = async () => {
    ...
    try {
      // Flatten rules
      const rulesToSave: AvailabilityRuleInput[] = [];
      Object.values(editingRules).forEach((dayRules) => {
        rulesToSave.push(...dayRules);
      });

      await updateAvailabilityRules({ rules: rulesToSave });
  ...
  const handleUpdateRule = (
    ...
    value: AvailabilityRuleInput[keyof AvailabilityRuleInput],
  ) => {
    const updated = { ...editingRules };
    updated[dayOfWeek][index] = {
      ...updated[dayOfWeek][index],
      [field]: value,
    };
  ...
  const handleRemoveRule = (dayOfWeek: number, index: number) => {
    const updated = { ...editingRules };
    updated[dayOfWeek].splice(index, 1);
    setEditingRules(updated);
  };
  ...
  const handleSaveConfig = async () => {
    ...
    try {
      const updated = await patchMyDoctorSchedulingConfig({
  ...
  const handleCreateException = async () => {
    ...
    try {
      const payload: AvailabilityExceptionCreateRequest = {
        date: exceptionDate,
        type: exceptionType,
      };

      if (exceptionType === 'custom') {
        payload.customWindows = customWindows;
      }
